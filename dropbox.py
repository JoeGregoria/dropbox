...
Usage for "zipping and uploading":
 python3 dropbox_upload.py -z -l info -s '/Volumes/xx/yy' -d '/dropbox/path/to/the.zip' -o file.json
...
APP_KEY = "GET-YER-OWN-APP-KEY"
TIMEOUT = 900
CHUNK_SIZE = 4 * 1024 * 1024
# and probably you should use the name of your program, not mine, for the configuration file.
my_config_file = os.path.join(os.path.expanduser("~"), ".dropbox_example.cfg")
refresh_token = None
...
def add_logging_arg(parser):
    parser.add_argument("--logging_level", "-l", type=str, help="error, warning, info, debug", default="warning", choices={"error", "warning", "info", "debug"})


def get_refresh_token(interactive: bool = None):
    """
...
def make_user_login_to_get_tokens():
    ...
    try:
        ...
        logger.error(f"error calling dropbox.DropboxOAuth2FlowNoRedirect.finish({auth_code})")
        logger.error(ex)
...
def upload(file_path: str = None, target_path: str = None, dbx=None, autorename: bool = False):
...
def agc_upload(
    ...
    if not refresh_token:
        ...
        refresh_token = get_refresh_token(interactive=interactive)
    else:
    ...
    with dropbox.Dropbox(oauth2_refresh_token=refresh_token, app_key=APP_KEY) as dbx:
        try:
            if use_team_root:
                ...
                #  and https://www.dropboxforum.com/t5/Dropbox-API-Support-Feedback/Folders-that-are-visible-from-the-SDK-don-t-appear-to-be-visible/td-p/738326
            ...
            if password:
                ...
                shared_link_settings = dropbox.sharing.SharedLinkSettings(
                    require_password=True, link_password=password, allow_download=True, expires=expiration, audience=dropbox.sharing.LinkAudience.public
                )
                ...
                return_value = shared_link_metadata.url
...
def retry_download_file(dbx, dropbox_path, local_file_path, max_retries=10, backoff=2):
    """Download a file from Dropbox with retries on failure."""
    for attempt in range(max_retries):
        try:
            download_file(dbx, dropbox_path, local_file_path)
            return  # Success
        except Exception as e:
            logger.warning(f"Download failed ({attempt + 1}/{max_retries}) for {dropbox_path}: {e}")
...
def download_folder(dbx, folder_path, local_dir):
    """Download an entire folder from Dropbox to a local directory with retry logic for files.
    Skips files that already exist in the local target directory."""
    total_files = 0
    total_folders = 0
            ...
            for entry in response.entries:
                if isinstance(entry, dropbox.files.FileMetadata):
                    ...
                    if os.path.exists(local_file_path):
                        ...
                        if hasattr(entry, 'size') and local_size == entry.size:
                            logger.info(f"File already exists and matches size, skipping: {local_file_path}")
                            continue
                        else:
                            logger.info(f"File exists but size mismatch; re-downloading: {local_file_path}")
                    logger.info(f"Downloading file {local_file_path}")
                    retry_download_file(dbx, entry.path_lower, local_file_path)
                    total_files += 1
                elif isinstance(entry, dropbox.files.FolderMetadata):
                    new_local_dir = os.path.join(local_dir, entry.name)
                    logger.info(f"Processing folder {new_local_dir}")
...
def agc_download_directory(dropbox_path: str = None, local_dir: str = None, interactive: bool = False, use_team_root: bool = True):
    global refresh_token
    if not refresh_token:
        logger.debug("getting refresh token")
        refresh_token = get_refresh_token(interactive=interactive)
...
def generate_password(pwlen: int = 8):
    """
    Create a very insecure password.

    See https://docs.python.org/3/library/secrets.html#recipes-and-best-practices for more info
    """
    alphabet = string.hexdigits
    password = "".join(secrets.choice(alphabet).lower() for i in range(pwlen))  # for a 20-character password
    return password
...
def convert_dropbox_url_into_download_only(the_url):
    ...
    # Step 2: Parse the query parameters
...
def do_upload(
        ...
        destination: str = None,
        dropbox_input_path: str = None,
        expiration_days: int = None,
        is_zip: bool = None,
        password: str = None,
        source: str = None,
        use_team_root: bool = False,
    ...
    if is_zip:
        if not dropbox_input_path:
            raise ValueError("Must specify source for zip to work")
        if zip_file_path:
            output_path = os.path.abspath(zip_file_path)
        else:
        ...
        if not password:
            password = generate_password(pwlen=pwlen)
    ...
    try:
        upload_return = agc_upload(source=dropbox_input_path, destination=destination, use_team_root=use_team_root, password=password, expiration=expiration_date, autorename=auto_rename)
    except Exception as e:
        logger.error(e)
        sys.exit(42)

    if is_zip:
        download_only_url = convert_dropbox_url_into_download_only(upload_return)
        output_json = {
        ...
        output_json = {"path": upload_return}
...
def main():
    ...
    parser.add_argument("-s", "--source", default="", required=False, help="Source file to copy from on computer"),
    parser.add_argument("-d", "--destination", default="", required=False, help="Destination folder path in DropBox")
    parser.add_argument("-a", "--authenticate", action="store_true", help="Only authenticate without transferring")
    parser.add_argument("-ar", "--auto_rename", action="store_true", help="If there is a conflict in the target filename, " "have the dropbox API auto-rename the file in dropbox.")
    ...
    parser.add_argument("--pwlen", "--password-length", dest="pwlen", default=20, help="When generating a password, use this length (defaults to 20)")
    ...
    parser.add_argument(
        ...
        "--zip_file_path",
        required=False,
        help="if -z, use this as the full zip file path " "(including the name of the zip file itself) " "like /nfs/agc-turbo/temp_zip/123456-AB.zip. If absent, defaults to ./<source>.zip",
    )
    parser.add_argument("-o", "--output", default="", required=False, help="Where to write a JSON of the Dropbox URL and password " "that get generated during a -z run")
    ...
    use_team_root = args.root == "team"

    dropbox_input_path = args.source
    password = args.password
    if args.pwlen is not None:
        pwlen = int(args.pwlen)
    ...
    output_json = do_upload(
        auto_rename=auto_rename,
        destination=destination,
        dropbox_input_path=dropbox_input_path,
        expiration_days=expiration_days,
        is_zip=is_zip,
        password=password,
        source=source,
        use_team_root=use_team_root,
